...
public class JFXCustomDialogLayout extends VBox {
     ...
     */
    public JFXCustomDialogLayout(boolean noHeading, boolean noBodyPadding, boolean noButtons) {
        initialize();
        heading.getStyleClass().addAll("jfx-layout-heading", "title");
        if (noHeading) {
            heading.setVisible(false);
            heading.setManaged(false);
            VBox.setMargin(body, new Insets(16, 0, 0, 0));
        }
        body.getStyleClass().add("jfx-layout-body");
        if (noBodyPadding) {
            body.setStyle("-fx-padding: 0 0 0 0");
        }
