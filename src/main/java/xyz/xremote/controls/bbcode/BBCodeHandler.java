package xyz.xremote.controls.bbcode;
...
public interface BBCodeHandler {
     ...
     */
    void startTag(String name, Map<String, String> params, int start, int length);
    ...
    class Default<T extends Pane> implements BBCodeHandler {
        ...
        @Override
        public void startTag(String name, Map<String, String> params, int start, int length) {
        ...
        protected Tag createTag(String name, Map<String, String> params) {
            ...
            switch (name) {
                // == TEXT STYLE ==
                case "b" -> {
                    stylesClass.add(TEXT_BOLD);
                    tagType = Tag.Type.TEXT;
                }
                case "i" -> {
                    stylesClass.add(TEXT_ITALIC);
                    tagType = Tag.Type.TEXT;
                }
                case "u" -> {
                    stylesClass.add(TEXT_UNDERLINED);
                    tagType = Tag.Type.TEXT;
                }
                case "s" -> {
                    stylesClass.add(TEXT_STRIKETHROUGH);
                    tagType = Tag.Type.TEXT;
                }
                case "color" -> {
                ...
                case "small" -> {
                    stylesClass.add(TEXT_SMALL);
                    tagType = Tag.Type.TEXT;
                }

                // LINKS
                case "url", "character", "person", "anime", "manga", "ranobe", "spoiler", "email" -> {
        ...
        protected Node createTextNode(Tag tag, String text) {
            return switch (tag.name()) {
                ...
                case "character" -> {
                    var node = new Hyperlink(text);
                    node.setUserData(tag.getParam("character"));
                    node.getStyleClass().add("character");
                    yield node;
                }
                case "person" -> {
                    var node = new Hyperlink(text);
                    node.setUserData(tag.getParam("person"));
                    node.getStyleClass().add("person");
                    yield node;
                }
                case "anime" -> {
                    var node = new Hyperlink(text);
                    node.setUserData(tag.getParam("anime"));
                    node.getStyleClass().add("anime");
                    yield node;
                }
                case "manga" -> {
                    var node = new Hyperlink(text);
                    node.setUserData(tag.getParam("manga"));
                    node.getStyleClass().add("manga");
                    yield node;
                }
                case "ranobe" -> {
                    var node = new Hyperlink(text);
                    node.setUserData(tag.getParam("ranobe"));
                    node.getStyleClass().add("ranobe");
                    yield node;
                }
                case "spoiler" -> {
                    var node = new Hyperlink(Optional.ofNullable(tag.getParam("spoiler")).orElse("spoiler"));
                    node.setUserData(text);
                    node.getStyleClass().add("spoiler");
                    yield node;
                }
                case "abbr" -> {
        ...
        protected <E extends Enum<E>> E getEnumValue(Class<E> c, String value, E defaultValue) {
            if (value == null) {
                return defaultValue;
            }
        ...
        protected int getListStartNumber(String param) {
            int start = 1;
     ...
     */
    record Block(Pane node, TextFlow text) {
