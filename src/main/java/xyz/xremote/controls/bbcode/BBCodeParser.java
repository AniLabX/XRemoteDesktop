...
public class BBCodeParser {
    ...
    public static final Set<String> RESERVED_TAGS = Set.of(
            ...
            "list", "media", "plain", "pre", "quote", "stop", "table", "tooltip",
            "td", "th", "tr", "warning",
            // Shikimori
            "spoiler", "character", "anime", "manga", "ranobe", "person"
    );
    ...
    public static String addColorAndTextSizeBBCodes(String color, float sizeEm, String value) {
        return "[color=" + color + "][size=" + sizeEm + "em]" + value + "[/size][/color]";
    }
     ...
     */
    public BBCodeParser(String input, BBCodeHandler handler, Set<String> tags) {
        this.input = getSpoilerReadyInput(input);
    ...
    private String getSpoilerReadyInput(String input) {
        ...
        for (int i = 0; i < 99; i++) {
            input = replaceBBTagsInsideSpoilerToCurlyBrackets(input);
        }

        return input.replace(" --carriage return-- ", "\r")
                .replace(" --linebreak-- ", "\n")
    ...
    private String replaceBBTagsInsideSpoilerToCurlyBrackets(String input) {
        return input.replaceAll("\\[spoiler(.*?)\\[(.*?)](.*?)spoiler]", "[spoiler$1{{$2}}$3spoiler]");
    }
    ...
    public static VBox createLayout(String input) {
        ...
        return createLayout(input, vBox);
    }
    ...
    public static <T extends Pane> T createLayout(String input, T container) {
        try {
            ...
            new BBCodeParser(input, handler).parse();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
        return container;
