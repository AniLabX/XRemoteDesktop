package xyz.xremote.controller;
...
import xyz.xremote.controls.bbcode.BBCodeParser;
import xyz.xremote.controls.preferences.*;
import xyz.xremote.layout.InputGroup;
import xyz.xremote.managers.LocaleManager;
import xyz.xremote.managers.Settings;
import xyz.xremote.managers.UpdatesChecker;
...
public class MainController {
    ...
    // Bonjour
    @FXML
    private InputPreference ipAppName;
    ...
    // MPV Arguments Warning
    @FXML
    private VBox vbMPVArgumentsWarn;

    // User arguments
    @FXML
    private InputPreference ipUserArguments;
    ...
    private void configureSettings() {
        ...
        configureUserArguments();
    }

    private void configureAppNameSection() {
        ipAppName.setText(Settings.XRemote.getAppName());
        ipAppName.textProperty().addListener((observable, oldValue, newValue) -> Settings.XRemote.putAppName(newValue));
    }
    ...
    private void configureUserArguments() {
        BBCodeParser.createLayout(BBCodeParser.addColorAndTextSizeBBCodes("white", .9f, LocaleManager.getString("gui.settings.player.mpv_arguments_warn")), vbMPVArgumentsWarn)
                .addEventFilter(
                        ActionEvent.ACTION,
                        event -> {
                            if (event.getTarget() instanceof Hyperlink link) {
                                FXUtils.openUrl((String) link.getUserData());
                            }
                            event.consume();
                        }
                );
        ipUserArguments.setText(Settings.Player.getUserArguments());
        ipUserArguments.textProperty().addListener((observable, oldValue, newValue) -> Settings.Player.putUserArguments(newValue));
    }
}