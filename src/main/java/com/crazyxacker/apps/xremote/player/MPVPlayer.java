...
import com.crazyxacker.apps.xremote.dialog.DialogBuilder;
import com.crazyxacker.apps.xremote.dialog.JFXCustomDialog;
import com.crazyxacker.apps.xremote.helpers.JavaHelper;
import com.crazyxacker.apps.xremote.helpers.OkHttpHelper;
import com.crazyxacker.apps.xremote.helpers.SevenZip;
...
public class MPVPlayer {
    ...
    @Getter
    private static XRemoteData remoteData;
    ...
    public static boolean play(XRemoteData remoteData, boolean killIfPresent, PlayerCallback callback) {
        MPVPlayer.remoteData = remoteData;
        String title = remoteData.getContentTitle() + " - " + remoteData.buildEpisodeTitle();
        return play(
                remoteData.getVideoUrl().getLinkPartsList().get(0),
                getSubtitlesUrl(remoteData.getParseLink()),
                title,
                Constants.USER_AGENT,
                remoteData.getParseLink().getHeadersForVideo(),
                remoteData.getContentId(),
                remoteData.getEpisodeId(),
                killIfPresent,
                callback
        );
    }
